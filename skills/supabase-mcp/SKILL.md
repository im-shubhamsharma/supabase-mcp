...
| `supa-mcp doctor` / `supa-mcp install` | Check deps / put `supa-mcp` on PATH. |

## How to drive it

- Prefer the `--json` output of `account list`, `projects`, and `status` so you can present
  clean choices in chat instead of pasting raw tool output.
...
- After a `link`, remind the user to start a fresh Claude Code session in that directory,
  and that the first connect shows a trust prompt plus a one-time Keychain prompt.
- When a user is unsure whether a directory is on the right account, run `supa-mcp status`.
  ...
  account (wrong account); recommend re-linking.

The slash commands `/supa-add`, `/supa-link`, `/supa-status`, `/supa-list`, and
`/supa-unlink` wrap these steps.
